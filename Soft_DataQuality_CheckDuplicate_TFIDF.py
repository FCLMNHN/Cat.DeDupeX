...
# NOTE: REMPLACER LA CLÉ API PAR VOTRE PROPRE CLÉ LORS DE L'UTILISATION RÉELLE
...
# ==============================================================================
# PARTIE 1 : LOGIQUE DE DÉDUPLICATION (Fonctionnels)
# ==============================================================================
...
# NOUVELLE FONCTION POUR LE CALCUL DE SIMILARITÉ TF-IDF (Étape 2)
...
def compare_chunk(
    ...
    for i, j in indices_pairs:
        ...
        # NOUVELLE RÈGLE : TF-IDF (Similarité Cosinus) - Analogue à la règle Forte
        if 'TFIDF' in enabled_rules:
            ...
            if is_desc1_significant and is_desc2_significant:
                ...
                # CORRECTION : Vérification de l'asymétrie
            ...
            with ProcessPoolExecutor(max_workers=NUM_PROCESSES) as executor:
                ...
                for i, future in enumerate(as_completed(futures)):
                    ...
                    for i_pair, j_pair, criteria, score in local_doubles:
                        # LOGIQUE DE DÉTERMINATION DU NIVEAU MISE À JOUR (Étape 6)
...
# ==============================================================================
# PARTIE 3 : DIALOGUE DE CONFIGURATION (Modifiée pour inclure TFIDF)
# ==============================================================================
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, current_settings: Dict[str, Any], parent=None):
        ...
        # self.setFixedSize(400, 300) # CODE ORIGINAL REMPLACÉ
        self._setup_ui()
        # NOUVEAU: Contrainte de taille après la mise en place du layout
    ...
    def _setup_ui(self):
        ...
        self.checkboxes = {}
        # MODIFICATION : Ajout de la règle 'TFIDF' (Étape 5)
    ...
    def get_settings(self) -> Dict[str, Any]:
        ...
        new_enabled_rules = []
        # La boucle parcourt désormais 'TOTAL', 'FORTE', 'PROBABLE', 'TFIDF'
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.settings: Dict[str, Any] = {
            # MODIFICATION : Ajout de 'TFIDF' dans les règles activées par défaut (Étape 4)
    ...
    # --- NOUVELLE MÉTHODE : S'assurer que les colonnes *_CLEAN existent ---
    ...
    def _setup_ui(self):
        ...
        # FIX DE HAUTEUR : Hauteur minimale et maximale (désactivé pour compatibilité multiplateforme DPI)
        MIN_HEIGHT = 150 # Estimer une hauteur raisonnable en pixels
        # ROW_HEIGHT_ESTIMATE = 25 
        # HEADER_HEIGHT_ESTIMATE = 30
        
        # MIN_HEIGHT = (ROW_HEIGHT_ESTIMATE * 5) + HEADER_HEIGHT_ESTIMATE # CODE ORIGINAL REMPLACÉ
        self.report_table.setMinimumHeight(MIN_HEIGHT)
        
        # MAX_HEIGHT = (ROW_HEIGHT_ESTIMATE * 15) + HEADER_HEIGHT_ESTIMATE # CODE ORIGINAL REMPLACÉ
        # self.report_table.setMaximumHeight(MAX_HEIGHT) # CODE ORIGINAL REMPLACÉ 
        ...
        # splitter.setSizes([MIN_HEIGHT * 2, 300]) # CODE ORIGINAL REMPLACÉ
        splitter.setSizes([300, 500]) # NOUVEAU: Initialisation plus générique 
    ...
    def catalog_loaded_result(self, success: bool, result: Optional[pd.DataFrame], message: str):
        ...
        if success and result is not None:
            ...
            # --- CORRECTIF : Assurer les colonnes de nettoyage après un nouveau téléchargement ---
    ...
    def update_active_catalog(self):
        ...
        if new_key and new_key in self.catalogs:
            ...
            # --- CORRECTIF : S'assurer que le DF actif a les colonnes nettoyées après un changement ---
    ...
    def start_deduplication_analysis(self):
        ...
        # Le nettoyage des données (création des colonnes *_CLEAN) est désormais géré
        # lors du chargement ou de la sélection du catalogue.
    ...
    def unpublish_dataset(self, datasetid: Optional[str], line_num: Optional[int]):
        ...
        if reply == QMessageBox.Yes:
            ...
            # On ne peut pas facilement désactiver les boutons ici, mais on se fie au contrôle du worker
            # et on désactive les boutons de contrôle principaux pour ne pas lancer d'autre analyse
            self.run_button.setEnabled(False)
            self.download_button.setEnabled(False)
    ...
    def show_comparison_view(self, index: QModelIndex, previous_index: QModelIndex = None):
        ...
        try:
            ...
            # Mise à jour des libellés de bouton
            ...
            # NOUVEAU STYLE pour forcer le saut de ligne à l'intérieur du <pre>
            # white-space: pre-wrap; permet de préserver les sauts de ligne existants tout en wrappant.
            # overflow-wrap: break-word; et word-break: break-all; forcent le wrapping des chaînes longues.
...
if __name__ == "__main__":
    # Correction: Ajout de QLayout à la liste des imports PySide6 au début du fichier
    ...
    # Rappel : Nécessite l'installation de scikit-learn
